...
- [ ] 12.1 צומת `Baseline Specialist`: מומחה לייצור "ספר חוקים" (Profile) מתוך לוגי ה-Golden.
- [ ] 12.2 צומת `Investigator Agent`: סוכן שמשתמש בכלים (RAG + Stats) כדי לחקור שגיאות מול ה-Baseline.
- [ ] 12.3 צומת `Supervisor`: מנהל העבודה שמנתב משימות ומרכז את התוצרים הסופיים.
...
- [ ] 13.1 הוספת פקודת CLI מרכזית `analyze all`.
- [ ] 13.2 הזרמת הלוגים החדשים ל-Investigator בחלקים (Chunks/Batches) במקביל, ואיסוף הדוח השלם על ידי ה-Supervisor.
...
- [ ] 14.1 עדכון `ai_service.py` לשימוש ב-Factory Pattern (מעבר בין מודלים לפי קובץ `.env`).